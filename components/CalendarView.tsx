      ...
      <div className="p-3 sm:p-6 flex flex-col sm:flex-row justify-between items-center border-b border-slate-100 dark:border-slate-800 gap-2 sm:gap-4">
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800 p-0.5 rounded-lg w-full sm:w-auto">
          ...
          <button
            ...
            className={`flex-1 sm:flex-none px-3 py-2 sm:px-3 sm:py-1.5 min-h-[44px] sm:min-h-0 rounded-md flex items-center justify-center gap-1.5 text-xs sm:text-sm font-medium transition-all ${viewMode === 'list' ? 'bg-white dark:bg-slate-700 shadow-sm text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
          >
            <ListIcon className="h-4 w-4 sm:h-3.5 sm:w-3.5" />
          ...
          <div key={`month-${currentDate.getFullYear()}-${currentDate.getMonth()}`} className="animate-fade-in">
            ...
            <div className="grid grid-cols-7 border-t border-l border-slate-100 dark:border-slate-800">
              {calendarDays.map((day, idx) => {
                if (!day) return <div key={`empty-${idx}`} className="min-h-[7rem] sm:min-h-[8rem] bg-slate-50/50 dark:bg-slate-800/30 border-b border-r border-slate-100 dark:border-slate-800"></div>;
                ...
                return (
                  <div key={day.toISOString()} className={`min-h-[7rem] sm:min-h-[8rem] group border-b border-r border-slate-100 dark:border-slate-800 p-1.5 sm:p-2 transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/50 ${isToday ? 'bg-brand-50 dark:bg-brand-900/50 ring-2 ring-brand-500 dark:ring-brand-400 ring-inset shadow-sm dark:shadow-brand-900/20' : 'bg-white dark:bg-slate-900/0'}`}>
                    <div className={`flex items-center justify-between gap-1 mb-1.5 sm:mb-2 ${isToday ? 'text-brand-700 dark:text-brand-300' : 'text-slate-400 group-hover:text-slate-600 dark:text-slate-500'}`}>
                            ...
                            {dayEvents.slice(0, 6).map(ev => {
                              ...
                              return (
                                <div
                                  ...
                                  className={`w-2 h-2 rounded-full ${colorClass} cursor-pointer transition-transform hover:scale-125`}
                                  title={ev.title}
                                />
                              );
                            })}
